				...
				details: {
					renderer: function ( api, rowIdx, columns ) {
						var data = $.map( columns, function ( col, i ) {
							...
							}else if(col.title == 'NotesCont'){
								if(col.hidden){
									return 	'<tr data-dt-row="'+col.rowIndex+'" data-dt-column="'+col.columnIndex+'" class=\'hidden invisible\'>'+
							...
							<table id="twittertbl" class="twittertbl display nowrap table-bordered table-togglable table-striped table-hover table-xxs" cellspacing="0" width="100%">
								<thead>
									<tr>
										...
										<th class="text-center none">NotesCont</th>
									</tr>
								</thead>
								<tbody>
								<?php
								...
								while($tweets = mysqli_fetch_array($sql_tweets)){
									...
									?>
										<tr class='<?php print $tweets["entry_type"]; ?>'>
											...
											<td id='<?php print $i.'-hiddenNotesCont';?>'><span id='<?php print $i.'-notes2';?>' class='invisible hidden'>
												<?php
													if(!$tweets["notes"]){print '{}';}else{print $tweets["notes"];}
												?>
											</span></td>
